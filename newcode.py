...
print("press Q to quit any part of shopping")
...
while True:
    ...
    if user_choice in inventory:
        ...
        quantity_input = input(f"In how much amount do you want {user_choice}:  ").strip()
        ...
        if quantity_input.isdigit() and int(quantity_input) > 0:
            ...
            if user_choice in cart:
                # 3. Cleaned up variables to make sure they all use integers
                cart[user_choice]['quantity'] += quantity
                cart[user_choice]['total_price'] += item_total_price
...
if not cart:
    print("your cart is empty your total bill is 0")
